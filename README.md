...
This Terraform workspace deploys Kubernetes configuration and controllers for the platform EKS cluster.
...
```
┌─────────────────────────────────────────────────────────────┐
│ HCP Terraform Remote State (platform-workloads-infrastructure)│
│ ├── VPC Configuration                                       │
│ ├── EKS Cluster Details                                     │
...
1. **Infrastructure Workspace Deployed**: The `platform-workloads-infrastructure` workspace must be successfully deployed
2. **HCP Terraform Token**: Set `TFE_TOKEN` environment variable or configure in `~/.terraform.d/credentials.tfrc.json`
